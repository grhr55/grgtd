...
useEffect(() => {
  fetch("https://frfrf-2zok.onrender.com/newsi/newosti")
    ...
    .then((data) => {
      setnews(data);
      console.log(Array.isArray(news), news);
      console.log(news , 'danis');
...
<div>
  ...
  {isLoading ? Array.from({ length: visibleCount }).map((_, index) => (
...
<div className="flex justify-center 2xl:mx-[90px] xl:mx-[170px] min-[1000px]:mx-[170px] max-[1200px]:mx-[0px] flex-wrap mt-[50px] min-[500px]:gap-[22px] max-[500px]:gap-[33px]">
  ...
  {(filteredfilm && filteredfilm.length > 0)
    ? filteredfilm.slice(0, visibleCount).map((product, index) => (
         <Link
          href={`/kino/${slugify(product.name)}`}
          key={product._id}
        >
    ...
    : Array.from({ length: 4 }).map((_, index) => (
        <div key={index} className="animate-pulse">
          ...
          <div className="h-5 bg-[#adacac] rounded mt-[10px]
                          2xl:w-[320px] xl:w-[200px] min-[500px]:w-[200px] max-[1300px]:w-[150px]" />
